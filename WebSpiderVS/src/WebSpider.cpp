	...
	try {
		...
		if (status_code == 200)
		{
			...
			for (unsigned int i = 0; i < parseResult.size(); i++) {
				// ignore new absolute links and links with "#"
				if (parseResult.at(i).find("://") == string::npos && parseResult.at(i).find("#") == string::npos) {
					parseResult.at(i) = "/" + parseResult.at(i);

					// check if the current link haven't been crawled yet
					for (unsigned int j = 0; j < crawledLinks.size(); j++) {
						if (parseResult.at(i) != crawledLinks.at(j)) {
#ifdef DEBUG
							cout << "New crawlable link found: " << parseResult.at(i) << "\n";
#endif
							// crawl new link
